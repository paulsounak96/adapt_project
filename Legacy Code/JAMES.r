...
dpower <- data.frame(3:20 * 0.01)
dpower.k <- data.frame(3:20 * 0.01)
dpower.a <- data.frame(3:20 * 0.01)
...
for(t in 1:100){
  ...
  mod.1 <- lm(y ~ ti_X)
  p <- summary(mod.1)$coefficients[,4]  
  ...
  for(i in 3:20){
    rej <- res$rejs[[i]]
    power <- c(power, sum(rej %in% nonzero) / length(nonzero))
  }
  ...
  dpower <- data.frame(dpower, power)
...
t.fdr <- dpower[,1]
dpower <- apply(dpower[,-1],1,mean)
dpower.k  <- apply(dpower.k[,-1],1,mean)
dpower.a <- apply(dpower.a[,-1],1,mean)



df <- data.frame(target.fdr = t.fdr, power.Kada = dpower, power.Knockoff = dpower.k, power.adapt = dpower.a)
