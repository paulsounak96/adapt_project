...
outfile="results8_paral.csv"
ntrials = 25
for(j in 1:ntrials){
  start <- Sys.time()
  print(j)
  for(i in 1:nrow(iter_list)){
    n = iter_list[i,1]    
    p = n*iter_list[i,2]     
    s = iter_list[i,3]
    rho = iter_list[i,4]
    ...
    res <- cbind(dat,n,p,s,amplitude,rho,j,ntrials)
    end <- Sys.time()
    print(end - start)
    
    if(file.exists(outfile)){
      write.table(res, outfile, sep = ",",append = T,row.names=F,col.names=F)
    }
    else{
      write.table(res, outfile, sep = ",",row.names=F,col.names=T)
    }
